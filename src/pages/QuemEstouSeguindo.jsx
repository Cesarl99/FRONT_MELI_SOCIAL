...
function QuemEstouSeguindo() {
  ...
  async function buscarSeguindo(ordenacaoAtual) {
    if (!usuarioLogado) {
    ...
    try {
      ...
      if (ordenacaoAtual == "A-Z") {
        url = `http://localhost:8080/users/${userId}/followed/list?order=name_asc`;
      } else {
        url = `http://localhost:8080/users/${userId}/followed/list?order=name_desc`;
      }
        ...
        <>
          ...
          <div style={{ borderTop: "1px solid #ccc", marginTop: 8 }}>
            {Array.isArray(dados.seguindo) && dados.seguindo.length > 0 ? (
              ...
              <p>Esse usuário não está seguido ninguem .</p>
            )}
          </div>
        </>
      )}
